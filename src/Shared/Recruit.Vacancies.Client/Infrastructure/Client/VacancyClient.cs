    ...
    : IRecruitVacancyClient, IEmployerVacancyClient, IJobsVacancyClient
{
    ...
    private static Guid GenerateVacancyId()
    {
        return Guid.NewGuid();
    }
    ...
    {
        return apprenticeshipRouteProvider.GetApprenticeshipRoutesAsync();
    }
    ...
    public async Task<PagedResults<VacancyApplication>> GetVacancyApplicationsAsync(VacancyApplicationsPagingParams pagingParams, long vacancyReference, bool vacancySharedByProvider)
    {
        long count = vacancySharedByProvider
            ? await applicationReviewRepository.GetCountForSharedVacancyAsync(vacancyReference)
            : await applicationReviewRepository.GetCountForVacancyAsync(vacancyReference);

        if (count == 0)
        {
            return new PagedResults<VacancyApplication>([], 0, 1, pagingParams.PageSize);
        }
            
        int maxPage = (int)Math.Ceiling((decimal)count / pagingParams.PageSize);
        int pageNumber = Math.Clamp(pagingParams.PageNumber, 1, maxPage);
        int skip = (pageNumber-1) * pagingParams.PageSize;
            
        var applications = vacancySharedByProvider
            ? await applicationReviewRepository.GetForSharedVacancyAsync(vacancyReference, pagingParams.SortColumn, pagingParams.SortOrder, skip, pagingParams.PageSize)
            : await applicationReviewRepository.GetForVacancyAsync(vacancyReference, pagingParams.SortColumn, pagingParams.SortOrder, skip, pagingParams.PageSize);
            
        return new PagedResults<VacancyApplication>(applications?.Select(c => (VacancyApplication)c) ?? [], Convert.ToInt32(count), pageNumber, pagingParams.PageSize);
    }
    ...
    {
        return apprenticeshipRouteProvider.GetApprenticeshipRouteAsync(routeId.GetValueOrDefault());
    }
